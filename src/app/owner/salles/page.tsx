...
export default function OwnerSallesPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Only fetch owner's own halls
      const { data } = await supabase.from('salles').select('*').eq('owner_id', user.id).order('created_at', { ascending: false });
      setSalles(data ?? []);
      setLoading(false);
    })();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        const { error } = await supabase.from('salles').update(payload).eq('id', editId);
        if (error) throw error;
        setSalles(prev => prev.map(s => s.id === editId ? { ...s, ...payload } as Salle : s));
      } else {
        const { data, error } = await supabase.from('salles').insert([payload]).select().single();
        if (error) throw error;
  ...
  if (loading) return (
    <div className="o-app">
      <OdooTopNav />
      <SubNav tabs={TABS} />
      <div className="o-actionbar"><div className="o-breadcrumb"><span className="o-breadcrumb-current">Mes salles des fêtes</span></div></div>
    ...
    <div className="o-app">
      ...
      <div className="o-actionbar">
        <div className="o-breadcrumb">
          <span className="o-breadcrumb-current">🏢 Mes salles des fêtes</span>
        </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h2 style={{ fontSize: 16, fontWeight: 700, color: '#212529', margin: 0 }}>
                {editId ? 'Modifier ma salle' : 'Nouvelle salle des fêtes'}
              </h2>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <input
                  type="checkbox"
                  id="owner_is_available"
                  checked={form.is_available}
                  onChange={e => setForm({ ...form, is_available: e.target.checked })}
                  style={{ width: 16, height: 16, accentColor: '#0D47A1' }}
                />
                <label htmlFor="owner_is_available" style={{ fontSize: 13, color: '#333', cursor: 'pointer' }}>
